...
AI Town 是一个很好的入口，但真正的工程问题会落在[编排层](/agent-engineering/orchestration-layer/)的状态、角色和验收，以及[OpenClaw 架构指南](/agent-frameworks/openclaw-architecture-guide/)里的控制面与执行面边界。把模拟当作实验系统，而不是演示视频，才值得继续扩展角色数量。
